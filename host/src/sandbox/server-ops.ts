...
export class SandboxServerOps {
  ...
  async handleExec(client: SandboxClient, message: ExecCommandMessage) {
    ...
    const payload = {
      ...
      isolate_proc: message.isolate_proc,
      deny_fork: message.deny_fork ?? false,
      allowed_readable_directories: message.allowed_readable_directories ?? [],
      allowed_writable_directories: message.allowed_writable_directories ?? [],
      cwd: message.cwd,
