...
7. Commit the version/changelog/lockfile changes, merge the reviewed release PR,
   then create and push the exact experimental tag.
8. Watch Experimental GitHub Release complete. It validates the same source
   commit, builds host/helpers/images, verifies the installed tarball and checksums,
   then publishes all assets in one GitHub prerelease. It does not publish npm
   packages or update main's registries. See `docs/fork-releases.md`.

For this fork, host and runner packages are private to prevent accidental upstream
npm publication. The release tarball removes optional krun packages and workspace
scripts. Internal package names remain unchanged for source compatibility.
Standalone helper/image workflows remain available; the coordinated prerelease
calls their build-only mode. Never retag or overwrite already published assets.
Qualification stays unverified until the released conformance bundle and required
runtime guarantees are independently validated.
