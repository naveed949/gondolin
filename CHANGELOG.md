...
- Cancel tracked host HTTP operations and await bounded dispatcher destruction
  ...
  callbacks retain failures; guest transport reconnection waits for the preceding
  HTTP generation to settle. This does not prove independent external teardown,
  undo remote effects, or implement response provenance and response-byte bounds.
